...
// ─────────────────────────────────────────────────────────────────────────────
// 릴레이 봉헌 신화 — 한 라운드는 하나의 seed 단어와 하나의 공유 장면(motif)으로
// MNEME(기원) → DOLON(절도/왜곡) → DEMOS(군중 확산) 순서로 이어 쓴다.
// 세 문단이 같은 사물(우물/달력/비석/불빛/문)을 공유하므로 합치면 하나의 짧은 신화가 된다.
//
// 규칙:
//  - "기록관은 ~했다" 같은 메타/기능 설명 금지. 세계 안에서 벌어진 '장면'으로 쓴다.
//  - 본문에 에이전트 이름·의미/색인/좌표 같은 메타 단어를 넣지 않는다.
//  - {word}는 seed 단어로 치환. 조사는 resolveJosa가 받침에 맞춰 정리한다.
// ─────────────────────────────────────────────────────────────────────────────
...
export const mythScenes: MythScene[] = [
  {
    mneme:
      "마을 북쪽 우물에는 아무도 마시지 않는 물이 있었다. 사람들은 그것을 '{word}'이라/라 불렀고, 해가 지기 전에는 그 이름을 입에 올리지 않았다.",
    dolon:
      "그러나 어느 밤, 누군가 우물 속에서 '{word}'의 마지막 음절을 훔쳐 갔다. 다음 날부터 사람들은 그 말을 할 때마다 서로의 비밀을 조금씩 대신 고백했다.",
    demos:
      "처음엔 한 노인이 우물 앞에서 그 말을 중얼거렸다. 곧 아이들이 장난처럼 따라 했고, 시장 상인들은 값을 부를 때마다 '{word}'을/를 같은 박자로 세 번씩 외쳤다. 그해 마을의 장례식은 모두 노래처럼 시끄러웠다."
  },
  {
    mneme:
      "가장 오래된 달력에는 비가 내리지 않는 날 하나가 비어 있었다. 사람들은 그 빈칸을 '{word}'이라/라 적었지만, 누구도 그 날을 본 적은 없었다.",
    dolon:
      "도둑은 그 빈칸에서 '{word}'을/를 꺼내고, 대신 굶주린 개 한 마리를 들여보냈다. 그래서 아이들은 그 말을 할 때마다 문밖을 먼저 살폈고, 어른들은 밥그릇을 하나 더 놓았다.",
    demos:
      "그 말은 오래 살아남았다. 비가 오기 전에도, 손님이 오기 전에도, 전쟁이 오기 전에도 사람들은 '{word}'을/를 불렀다. 그리고 매번 도착한 것은 그 말이 아니라, 함께 기다릴 수밖에 없는 어떤 것이었다."
  },
  {
    mneme:
      "산 어귀에는 글자가 닳은 비석이 서 있었다. 늙은이들은 거기 새겨진 말이 '{word}'이라/라 했고, 비석에 손을 대면 그해 농사를 점쳤다.",
    dolon:
      "어느 도둑이 밤에 그 비석의 획 하나를 긁어냈다. 그날 이후 '{word}'은/는 같은 소리로 읽혀도 반대의 뜻으로 이루어졌고, 축복을 빌던 자가 재앙을 받았다.",
    demos:
      "사람들은 비석 앞에서 '{word}'을/를 번갈아 외기 시작했다. 한 사람이 외면 열이 받고, 열이 외면 골짜기가 받았다. 이윽고 바람조차 그 말의 박자로 불었다."
  },
  {
    mneme:
      "강 건너에는 꺼지지 않는 불빛이 있었고, 사람들은 그것을 '{word}'이라/라 불렀다. 첫 사람이 그 이름을 입에 올렸을 때, 강물이 한 번 방향을 바꾸었다고 한다.",
    dolon:
      "도둑은 그 불빛에서 '{word}'의 첫 음절을 떼어 베개 밑에 숨겼다. 그날부터 아이들은 꿈에서 거짓 예언을 들었고, 아침이면 같은 말을 하고도 서로 다른 얼굴을 떠올렸다.",
    demos:
      "강 양쪽에서 사람들이 '{word}'을/를 주고받았다. 혼례에서도 장례에서도 같은 가락으로 울렸고, 뜻을 모르는 채 외우는 동안 사람들의 얼굴은 조금씩 서로 닮아 갔다."
  },
  {
    mneme:
      "오래된 집에는 절대 두드리지 않는 문이 하나씩 있었다. 사람들은 그 문을 '{word}'이라/라 불렀고, 아이가 태어나면 그 앞에서 이름을 먼저 고했다.",
    dolon:
      "도둑이 그 문에서 '{word}'의 빗장을 훔쳤다. 그 뒤로 문은 안에서 잠겼고, 사람들은 그 말을 할 때마다 저도 모르게 한 걸음 물러섰다.",
    demos:
      "누군가 문 앞에서 '{word}'을/를 외치자 옆집이 받고 윗집이 받았다. 저녁이 되자 지붕 위의 새들까지 그 말을 흉내 냈고, 마을은 밤새 같은 말을 돌려 불렀다."
  }
];

export function pickMythScene(seed: number): MythScene {
  const i = ((seed % mythScenes.length) + mythScenes.length) % mythScenes.length;
  return mythScenes[i];
}
